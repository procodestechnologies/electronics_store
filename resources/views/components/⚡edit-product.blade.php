...
new class extends Component {
    ...
    {
        $this->sku = $this->sku ?? $this->product->sku;
    ...
    public function updateProduct()
    {
        ...
        //     }
        // }
        Flux::toast(text: 'Product updated successfully!', variant: 'success');
        // $this->reset();
    }
    ...
    <form wire:submit.prevent="updateProduct">
        ...
        <flux:select class="mb-2" wire:model="brand_id" label="Brand">
            @foreach (\App\Models\Brand::all() as $brand)
                <flux:select.option {{ $brand->id === $brand_id ? 'selected' : '' }} value="{{ $brand->id }}">
                    {{ $brand->name }}</flux:select.option>
            @endforeach
        </flux:select>
